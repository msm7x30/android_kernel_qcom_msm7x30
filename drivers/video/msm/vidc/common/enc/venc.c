		...
		unsigned cmd, unsigned long u_arg)
{
	...
	{
		DBG("%s(): Executing VEN_IOCTL_CMD_PAUSE\n", __func__);
	...
	case VEN_IOCTL_SET_METABUFFER_MODE:
	{
		...
		if (vcd_status) {
			ERR("Setting metabuffer mode failed\n");
			return -EIO;
		}
		break;
	}
	...
	case VEN_IOCTL_SET_SLICE_DELIVERY_MODE:
	{
		...
		if (vcd_status) {
			ERR("Setting slice delivery mode failed\n");
			return -EIO;
		}
		break;
	}
	case VEN_IOCTL_SET_SPS_PPS_FOR_IDR:
	{
		...
		vcd_status = vcd_set_property(client_ctx->vcd_handle,
				&vcd_property_hdr, &idr_enable);
		if (vcd_status) {
			ERR("Setting sps/pps per IDR failed\n");
			return -EIO;
		}
		break;
	}
	case VEN_IOCTL_SET_VUI_BITSTREAM_RESTRICT_FLAG:
	{
		...
		if (vcd_status) {
			ERR("Setting bitstream restrict flag failed\n");
			return -EIO;
		}
		break;
	}
