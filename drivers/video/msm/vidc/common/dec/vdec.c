		   ...
		   void *info, size_t sz, void *handle, void *const client_data)
{
	...
	case VCD_EVT_IND_INFO_FIELD_DROPPED:
		if (info)
			...
			ERR("Wrong Payload for Field dropped\n");
		break;
...
static u32 vid_dec_set_turbo_clk(struct video_client_ctx *client_ctx)
{
	...
	if (vcd_status) {
		ERR("%s: set turbo perf_level failed\n", __func__);
		return false;
	} else {
		return true;
	}
			 ...
			 enum vdec_bufferflush flush_dir)
{
	...
	DBG("msm_vidc_dec: %s() called with dir = %u\n", __func__,
		 flush_dir);
	if (!client_ctx) {
		ERR("Invalid client_ctx\n");
		return false;
...
static int vid_dec_release_secure(struct inode *inode, struct file *file)
{
	...
	vidc_disable_clk();
#endif
	DBG("msm_vidc_dec: Return from %s()\n", __func__);
	return 0;
}
