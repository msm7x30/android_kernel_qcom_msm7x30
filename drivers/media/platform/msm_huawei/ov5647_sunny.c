...
static DEFINE_MUTEX(ov5647_sunny_mut);
...
struct register_address_value_pair const ov5647_sunny_preview[] = {
...
//15fps
/*========================================*/
struct register_address_value_pair const ov5647_sunny_snapshot[] = {
...
struct ov5647_sunny_reg ov5647_sunny_regs = {
	...
	.prev_reg_settings_size = ARRAY_SIZE(ov5647_sunny_preview),
	.snap_reg_settings = &ov5647_sunny_snapshot[0],
	.snap_reg_settings_size = ARRAY_SIZE(ov5647_sunny_snapshot),
};
...
static uint16_t ov5647_sunny_sw_damping_time_wait = 1;

/*=============================================================*/

static int ov5647_sunny_i2c_rxdata(
	unsigned short saddr, unsigned char *rxdata, int length)
{
	struct i2c_msg msgs[] = {
		{
			.addr = saddr,
...
static int32_t ov5647_sunny_i2c_read_w(
	unsigned short saddr, unsigned short raddr, unsigned short *rdata)
{
	int32_t rc = 0;
	unsigned char buf[4];
...
static int32_t ov5647_sunny_i2c_txdata(
	unsigned short saddr, unsigned char *txdata, int length)
{
	struct i2c_msg msg[] = {
		{
			.addr = saddr,
...
static int32_t ov5647_sunny_i2c_write(
	unsigned short saddr, unsigned short waddr, unsigned short wdata)
{
	int32_t rc = -EIO;
	unsigned char buf[3];
...
static int32_t ov5647_sunny_i2c_write_table(
	struct register_address_value_pair const *reg_conf_tbl, int num)
{
	int i;
	int32_t rc = 0;
...
static void ov5647_sunny_setup_af_tbl(void)
{
	...
	for (i = 2; i < OV5647_SUNNY_TOTAL_STEPS_NEAR_TO_FAR; i++) {
		if (i < ov5647_sunny_nl_region_boundary1)
			ov5647_sunny_pos_tbl[i] = ov5647_sunny_pos_tbl[i-1] +
				ov5647_sunny_nl_region_code_per_step1;
		else if (i <= ov5647_sunny_nl_region_boundary2)
			ov5647_sunny_pos_tbl[i] = ov5647_sunny_pos_tbl[i-1] +
				ov5647_sunny_nl_region_code_per_step2;
		else if (i <= ov5647_sunny_nl_region_boundary3)
			ov5647_sunny_pos_tbl[i] = ov5647_sunny_pos_tbl[i-1] +
				ov5647_sunny_nl_region_code_per_step3;
		else if (i <= ov5647_sunny_nl_region_boundary4)
			ov5647_sunny_pos_tbl[i] = ov5647_sunny_pos_tbl[i-1] +
				ov5647_sunny_nl_region_code_per_step4;
		else
			ov5647_sunny_pos_tbl[i] = ov5647_sunny_pos_tbl[i-1] +
				ov5647_sunny_nl_region_code_per_step5;
	}
}
...
static void ov5647_sunny_get_pict_fps(uint16_t fps, uint16_t *pfps)
{
	...
	CDBG("%s preview_fps = %d", __func__, fps);

	/* Total frame_length_lines and line_length_pck for preview */
	preview_frame_length_lines =
		OV5647_SUNNY_QTR_SIZE_HEIGHT + OV5647_SUNNY_VER_QTR_BLK_LINES;
	preview_line_length_pck =
		OV5647_SUNNY_QTR_SIZE_WIDTH + OV5647_SUNNY_HRZ_QTR_BLK_PIXELS;

	/* Total frame_length_lines and line_length_pck for snapshot */
	snapshot_frame_length_lines =
		OV5647_SUNNY_FULL_SIZE_HEIGHT + OV5647_SUNNY_VER_FULL_BLK_LINES;
	snapshot_line_length_pck =
		OV5647_SUNNY_FULL_SIZE_WIDTH + OV5647_SUNNY_HRZ_FULL_BLK_PIXELS;
...
{
	return (OV5647_SUNNY_FULL_SIZE_HEIGHT +
		OV5647_SUNNY_VER_FULL_BLK_LINES);
}

static uint16_t ov5647_sunny_get_pict_pixels_pl(void)
{
	return (OV5647_SUNNY_FULL_SIZE_WIDTH +
		OV5647_SUNNY_HRZ_FULL_BLK_PIXELS);
}

static uint32_t ov5647_sunny_get_pict_max_exp_lc(void)
{
	return (OV5647_SUNNY_FULL_SIZE_HEIGHT +
		OV5647_SUNNY_VER_FULL_BLK_LINES) * 2;
}
...
static int32_t ov5647_sunny_write_exp_gain(uint16_t gain, uint32_t line)
{
	...
	if (ov5647_sunny_ctrl->curr_res == QTR_SIZE) {
		min_lines_per_frame = OV5647_SUNNY_QTR_SIZE_HEIGHT +
			OV5647_SUNNY_VER_QTR_BLK_LINES;
		max_lines_per_frame = min_lines_per_frame * 4;
	} else {
		min_lines_per_frame =
			OV5647_SUNNY_FULL_SIZE_HEIGHT + OV5647_SUNNY_OFFSET;
		max_lines_per_frame = min_lines_per_frame * 4;
	}
	...
	CDBG("%s start gain=%d line = %d from_boundary = %d\n",
		__func__, gain, line, frame_boundary);
	gain_msb = (uint8_t) ((gain & 0xFF00) >> 8);
	gain_lsb = (uint8_t) (gain & 0xFF);
	if (!!ov5647_sunny_i2c_write(
		ov5647_sunny_client->addr, 0x350a, gain_msb))
		return CAMERA_FAILED;
	if (!!ov5647_sunny_i2c_write(
		ov5647_sunny_client->addr, 0x350b, gain_lsb))
		return CAMERA_FAILED;
	...
	if (!!ov5647_sunny_i2c_write(
		ov5647_sunny_client->addr,0x380e, boundary_msb))
		return CAMERA_FAILED;
	if (!!ov5647_sunny_i2c_write(
		ov5647_sunny_client->addr,0x380f, boundary_lsb))
		return CAMERA_FAILED;
	...
	enum ov5647_sunny_setting rt)
{
	...
	case UPDATE_PERIODIC:
		if (rt == RES_PREVIEW) {
			rc = ov5647_sunny_i2c_write_table(
				ov5647_sunny_regs.prev_reg_settings,
				ov5647_sunny_regs.prev_reg_settings_size);
			mdelay(10);
		} else {
			rc = ov5647_sunny_i2c_write_table(
				ov5647_sunny_regs.snap_reg_settings,
...
static int32_t ov5647_sunny_af_i2c_write(uint16_t data)
{
	...
	if (rc < 0) {
		CDBG("i2c_write failed, saddr = 0x%x addr = 0x%x, val = 0x%x\n",
			OV5647_SUNNY_AF_I2C_ADDR >> 1,
			code_val_msb, code_val_msb);
	}
	return rc;
}
...
static int32_t ov5647_sunny_move_focus(int direction, int32_t num_steps)
{
	...
	dest_step_position =
		ov5647_sunny_ctrl->curr_step_pos + (step_direction * num_steps);
	...
	target_dist = step_direction *
		(dest_lens_position - ov5647_sunny_ctrl->curr_lens_pos);
...
static int ov5647_sunny_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	ov5647_sunny_ctrl = kzalloc(
		sizeof(struct ov5647_sunny_ctrl), GFP_KERNEL);
...
static int ov5647_sunny_sensor_config(void __user *argp)
{
	...
	switch (cdata.cfgtype)
	{
		...
		case CFG_GET_PICT_MAX_EXP_LC:
			cdata.cfg.pict_max_exp_lc =
				ov5647_sunny_get_pict_max_exp_lc();
		...
		case CFG_MOVE_FOCUS:
			CDBG("ov5647_sunny_ioctl: CFG_MOVE_FOCUS: "
				"cdata.cfg.focus.dir=%d "
...
static int ov5647_sunny_sensor_release(void)
{
	int rc = -EBADF;
	...
	const struct i2c_device_id *id)
{
	...
	ov5647_sunny_sensorw = kzalloc(
		sizeof(struct ov5647_sunny_work), GFP_KERNEL);
