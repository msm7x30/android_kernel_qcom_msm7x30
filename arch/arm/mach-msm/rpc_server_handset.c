...
static int hs_probe(struct platform_device *pdev)
{
	...
	struct msm_handset_platform_data *pdata = dev_get_platdata(dev);
	...
	if (node) {
		pdata = devm_kzalloc(dev,
				     sizeof(struct msm_handset_platform_data),
				     GFP_KERNEL);
		if (!pdata) {
		...
		rc = of_property_read_string(node, "hs-name",
			&pdata->hs_name);
		if (rc) {
			dev_warn(dev, "failed to read 'hs-name' rc=%d\n",
				 rc);
			rc = 0; /* fall through */
		}

		rc = of_property_read_u32(node, "pwr-key-delay-ms",
			&pdata->pwr_key_delay_ms);
		if (rc) {
			dev_warn(dev,
				 "failed to read 'pwr-key-delay-ms' rc=%d\n",
				 rc);
			rc = 0; /* fall through */
		}
	}
	...
	if (rc) {
		dev_err(dev, "failed to register input device rc=%d\n",
			rc);
	...
	rc = hs_rpc_init();
	if (rc) {
		dev_err(dev, "rpc init failure rc=%d\n", rc);
...
static struct platform_driver hs_driver = {
	...
	.driver	= {
		.name		= DRIVER_NAME,
		.owner		= THIS_MODULE,
		.of_match_table	= of_match_ptr(hs_of_match_table),
	},
};
